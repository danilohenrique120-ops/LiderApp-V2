...
const Dashboard: React.FC<DashboardProps> = ({ matrixData, pdis, meetings, skills, procedures, trainingRecords, productionData, investigations }) => {
    
    const exportToPDF = (elementId: string, filename: string) => {
        const element = document.getElementById(elementId);
        // Fix: Cast window.html2pdf to any to resolve call signature error
    ...
    // Novo indicador: Aderência de Erro Humano (Prazos vs Hoje)
    const errorAdherence = useMemo(() => {
        if (investigations.length === 0) return 100;
        const today = new Date().toISOString().split('T')[0];
        const onTime = investigations.filter(inv => inv.actionPlan.deadline >= today).length;
        return Math.round((onTime / investigations.length) * 100);
    }, [investigations]);

    const countermeasureDistribution = useMemo(() => {
        const counts: Record<string, number> = {};
        investigations.forEach(inv => {
            const cm = inv.actionPlan.countermeasure;
    ...
    const rootCauseDistribution = useMemo(() => {
        const counts: Record<string, number> = {};
        investigations.forEach(inv => {
            const rc = inv.actionPlan.rootCauses || 'Não especificada';
    ...
    return (
        <div id="print-dashboard" className="animate-fade">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                ...
                <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm hover:shadow-md transition-all">
                    <div className="flex items-center justify-between mb-4">
                        ...
                        <span className="text-[10px] font-black text-slate-300 uppercase tracking-widest">Erro Humano</span>
                    </div>
                    <p className="text-4xl font-black text-red-600 tracking-tighter">{investigations.length}</p>
                    <p className="text-xs font-bold text-slate-400 mt-1 uppercase tracking-tighter">Aderência de Ações: {errorAdherence}%</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
                <div className="bg-slate-900 p-8 rounded-[2.5rem] text-white shadow-2xl">
                    <div className="flex items-center justify-between mb-6">
                        <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest">Contramedidas de Erro Humano</h3>
                        <ShieldAlert size={16} className="text-red-500" />
                    </div>
                    <div className="h-48">
                        <ResponsiveContainer width="100%" height="100%">
                            <PieChart>
                                <Pie data={countermeasureDistribution} cx="50%" cy="50%" innerRadius={40} outerRadius={60} paddingAngle={8} dataKey="value">
                                    {countermeasureDistribution.map((_, index) => (
                                        <Cell key={`cell-cm-${index}`} fill={['#3b82f6', '#10b981', '#f59e0b', '#ef4444', '#8b5cf6', '#ec4899'][index % 6]} />
                                    ))}
                                </Pie>
                    ...
                    <div className="grid grid-cols-2 gap-x-4 gap-y-2 mt-4">
                        {countermeasureDistribution.map((d, i) => (
                            <div key={i} className="flex items-center gap-2">
                                <div className="w-2 h-2 rounded-full" style={{backgroundColor: ['#3b82f6', '#10b981', '#f59e0b', '#ef4444', '#8b5cf6', '#ec4899'][i % 6]}}></div>
                                <span className="text-[8px] font-black uppercase text-slate-400 tracking-tighter truncate">{d.name}: {d.value}</span>
                            </div>
                        ))}
                    </div>
                </div>
                ...
                <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
                    <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-6">Últimos Feedbacks & PDI</h3>
                    <div className="space-y-4">
                        ...
                        <div className="pt-2">
                            <div className="flex justify-between items-end mb-2">
                                <span className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Conclusão de Metas PDI</span>
                                <span className="text-xl font-black text-blue-600">{pdiCompletion}%</span>
                            </div>
            ...
            {/* Novo Gráfico de Causas Raízes */}
            <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm animate-fade">
                <div className="flex items-center justify-between mb-8">
                    <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest">Top 5 Causas Raízes (Frequência)</h3>
                    <TrendingUp size={16} className="text-blue-500" />
                </div>
                <div className="h-64">
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={rootCauseDistribution} layout="vertical" margin={{ left: 30, right: 30 }}>
                            <CartesianGrid strokeDasharray="3 3" horizontal={true} vertical={false} stroke="#f1f5f9" />
                            ...
                            <Bar dataKey="value" fill="#3b82f6" radius={[0, 8, 8, 0]} barSize={20}>
                                {rootCauseDistribution.map((entry, index) => (
                                    <Cell key={`cell-rc-${index}`} fill={index === 0 ? '#ef4444' : '#3b82f6'} fillOpacity={1 - (index * 0.15)} />
                                ))}
                            </Bar>
                        </BarChart>
                    </ResponsiveContainer>
                </div>
                {rootCauseDistribution.length === 0 && (
                    <div className="text-center py-10">
                        <p className="text-[10px] font-black text-slate-300 uppercase italic tracking-widest">Aguardando dados de investigação...</p>
                    </div>
                )}
            </div>
        </div>
