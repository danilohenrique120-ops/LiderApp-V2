...
const TodoView: React.FC<TodoViewProps> = ({ folders, tasks, notes, user, db }) => {
    ...
    const filteredTasks = useMemo(() => {
        return tasks.filter(t => {
            ...
            // Aplicar o filtro de "Minimizar Concluídas" (Ocultar)
            const matchesVisibility = showCompleted ? true : !t.completed;

            return matchesSearch && matchesFolder && matchesPriority && matchesStatus && matchesVisibility;
        }).sort((a, b) => {
    ...
    }, [tasks, activeFolderId, searchQuery, statusFilter, priorityFilter, showCompleted]);
        ...
        <div className="flex flex-col lg:flex-row gap-8 animate-fade min-h-[calc(100vh-200px)]">
            ...
            <div className="flex-1 flex flex-col gap-8 min-w-0">
                ...
                <section className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-8 flex flex-col">
                    ...
                    <div className="flex flex-col gap-6 mb-10">
                        ...
                        <div className="flex flex-wrap items-center justify-between gap-4 p-4 bg-slate-50 rounded-[1.5rem] border border-slate-100">
                            <div className="flex items-center gap-2 overflow-x-auto custom-scrollbar pb-2 md:pb-0">
                                ...
                                {/* Novo Botão: Minimizar Concluídas */}
                                <div className="h-6 w-[1px] bg-slate-200 mx-2 hidden md:block"></div>
                                <button 
                                    onClick={() => setShowCompleted(!showCompleted)}
                                    className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap flex items-center gap-2 ${!showCompleted ? 'bg-blue-50 text-blue-600 border border-blue-200' : 'bg-white text-slate-400 border border-slate-200'}`}
                                    title={showCompleted ? "Ocultar Tarefas Concluídas" : "Mostrar Tarefas Concluídas"}
                                >
                                    {showCompleted ? <Eye size={12} /> : <EyeOff size={12} />}
                                    {showCompleted ? "Minimizar Concluídas" : "Mostrar Concluídas"}
                                </button>
                            </div>
                    ...
                    {/* Container de Lista com Barra de Rolagem (Mostra 5-6 tarefas) */}
