...
interface Message {
    ...
    role: 'user' | 'ai';
    text: string;
    image?: string; // Base64 para exibição no chat
    timestamp: Date;
}
...
const SUGGESTED_CHIPS = [
    ...
    { id: 2, text: "Auditoria 5S: Analise esta imagem em busca de desvios.", icon: Camera, color: "text-indigo-500", bg: "bg-indigo-50" },
...
const ComplianceView: React.FC<ComplianceViewProps> = ({ user, db, documents }) => {
    ...
    // Estados para Auditoria por Foto
    ...
    const handleSendMessage = async (text: string) => {
        ...
        const userMsg: Message = {
            ...
            role: 'user',
            text: text || (selectedImage ? "Auditoria por imagem solicitada." : ""),
            image: selectedImage ? `data:${selectedImage.mime};base64,${selectedImage.data}` : undefined,
            timestamp: new Date()
        };
        ...
        const currentImage = selectedImage; // Backup para a chamada da IA
        setSelectedImage(null); // Limpa o preview
        ...
        try {
            ...
            if (currentImage) {
                // Chama fluxo Multimodal
                responseText = await aiService.analyzeSafetyImage(
                    currentImage.data,
                    currentImage.mime,
                    text,
                    contextText
                ...
                // Chama fluxo de IA dedicado para Q&A dos documentos anexados
                const response = await aiService.queryKnowledgeBase(text, contextText);
                ...
                <div ref={scrollRef} className="flex-1 overflow-y-auto p-8 space-y-6 custom-scrollbar bg-slate-50/20">
                    {messages.length === 0 ? (
                        <div className="h-full flex flex-col items-center justify-center max-w-2xl mx-auto text-center animate-fade">
                            ...
                            <p className="text-slate-400 text-sm font-medium mb-12">Capture uma foto de um desvio ou selecione uma dúvida abaixo.</p>
                            
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 w-full">
                                {SUGGESTED_CHIPS.map(chip => (
                                    <button 
                                        key={chip.id}
                                        onClick={() => chip.id === 2 ? auditImageInputRef.current?.click() : handleSendMessage(chip.text)}
                        ...
                        messages.map((m) => (
                            <div key={m.id} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'} animate-fade group`}>
                                <div className="max-w-[85%] relative">
                                    {m.image && (
                                        <div className="mb-2 rounded-[2rem] overflow-hidden border-4 border-white shadow-lg">
                                            <img src={m.image} alt="Auditoria" className="max-w-full h-auto object-cover max-h-80" />
                                        </div>
                                    )}
                    ...
                    {isGenerating && (
                        <div className="flex justify-start animate-pulse">
                            <div className="bg-white border border-slate-100 p-6 rounded-[2.5rem] rounded-tl-none flex items-center gap-3">
                                ...
                                <span className="text-[10px] font-black text-slate-300 uppercase tracking-widest">Auditor Visual Analisando...</span>
                            </div>
                        </div>
                    )}
                </div>

                {/* Área de Input Fixo com Preview de Imagem */}
                <footer className="p-8 bg-white border-t border-slate-100">
                    {selectedImage && (
                        <div className="max-w-4xl mx-auto mb-4 flex items-center gap-4 p-4 bg-blue-50 rounded-[1.5rem] border border-blue-100 animate-fade">
                            <div className="relative w-20 h-20 rounded-xl overflow-hidden shadow-md">
                                <img src={`data:${selectedImage.mime};base64,${selectedImage.data}`} className="w-full h-full object-cover" alt="Preview" />
                                <button onClick={() => setSelectedImage(null)} className="absolute top-1 right-1 p-1 bg-red-500 text-white rounded-full"><X size={12}/></button>
                            </div>
                            <div className="flex-1">
                                <p className="text-[10px] font-black text-blue-600 uppercase tracking-widest">Imagem Pronta</p>
                                <p className="text-xs text-slate-500 font-bold">Descreva o que deseja analisar ou apenas envie.</p>
                            </div>
                        </div>
                    )}
                    ...
                    >
                        <button 
                            type="button" 
                            onClick={() => auditImageInputRef.current?.click()}
                            className={`p-4 rounded-full transition-colors ${selectedImage ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-400 hover:text-blue-500'}`}
                        >
                            <Camera size={20} />
                        </button>
                        <input 
                        ...
                        <input 
                            ...
                            placeholder={documents.length > 0 ? "Envie uma foto para auditoria ou faça uma pergunta..." : "Aguardando documentos técnicos..."}
                            value={input}
                            onChange={(e) => setInput(e.target.value)}
                            disabled={isGenerating || (documents.length === 0 && !selectedImage)}
                            onKeyDown={(e) => {
                        ...
                        <div className="flex items-center gap-4 pr-4">
                            <button 
                                type="submit"
                                disabled={isGenerating || (!input.trim() && !selectedImage) || (documents.length === 0 && !selectedImage)}
                    ...
                    <p className="text-center mt-4 text-[9px] font-black text-slate-300 uppercase tracking-[0.2em]">
                        Padrão de Auditoria Sistema Líder • Visão Artificial Multimodal
                    </p>
                </footer>
            </main>
        </div>
