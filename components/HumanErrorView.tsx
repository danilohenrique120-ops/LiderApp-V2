...
const HumanErrorView: React.FC<HumanErrorViewProps> = ({ investigations, user, db }) => {
    ...
    const handleGenerateAiReport = async () => {
        ...
        try {
            ...
            const prompt = `
                ...
                - Colaborador: ${occurrence.employee.name} (${occurrence.employee.function})
                ...
                - Causa Raiz: ${actionPlan.rootCauses}
                - Ação Corretiva: ${actionPlan.action}
                - Contramedida Principal: ${actionPlan.countermeasure}
                ...
                Use um tom profissional e direto. Formate o texto de maneira legível.
            `;
            ...
            alert("Falha ao chamar a API Gemini. Verifique sua conexão e se os dados estão preenchidos corretamente.");
        } finally {
    ...
    const handleEdit = (inv: HumanErrorInvestigation) => {
        ...
        // @ts-ignore
        setTwttpDate(inv.twttp.date || '');
        const cleanTwttp = { ...inv.twttp };
        // @ts-ignore
        delete cleanTwttp.date;
    ...
    const handleSave = async () => {
        ...
        try {
            if (editingId) {
                ...
                alert('Investigação atualizada com sucesso.');
            } else {
                                ...
                                <div className="border-t pt-8 mt-4">
                                    <div className="flex justify-between items-center mb-6">
                                        <h4 className="text-[10px] font-black text-blue-600 uppercase tracking-widest flex items-center gap-2">
                                            <Sparkles size={16} /> Central de Insights Analíticos (IA)
                                        </h4>
                                    ...
                                    {isAiLoading ? (
                                        <div className="bg-slate-50 p-12 rounded-[2.5rem] flex flex-col items-center gap-4 text-center border border-dashed border-blue-200 animate-pulse">
                                            <div className="w-10 h-10 border-4 border-blue-600 border-t-transparent rounded-full animate-spin"></div>
                                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">IA Analisando Causa Raiz...</p>
                                        </div>
                                    ) : aiReport ? (
                                        <div className="bg-slate-900 text-slate-200 p-8 md:p-10 rounded-[2.5rem] shadow-2xl relative animate-fade">
                                            <div className="absolute top-6 right-6 flex gap-2">
                                                <button onClick={handleCopyReport} className="p-2 bg-slate-800 text-white rounded-lg hover:bg-slate-700 transition-all" title="Copiar Texto">
                                                    {copied ? <Check size={16} className="text-emerald-500" /> : <Copy size={16} />}
                                                </button>
                                                <button onClick={() => setAiReport(null)} className="p-2 bg-slate-800 text-white rounded-lg hover:bg-red-500 transition-all">
                                                    <Trash2 size={16} />
                                                </button>
                                            </div>
                                            <div className="prose prose-invert prose-sm max-w-none">
                                                <div className="whitespace-pre-wrap font-medium leading-relaxed opacity-90 text-sm">
                                                    {aiReport}
                                                </div>
                                            </div>
                                            <div className="mt-8 pt-6 border-t border-slate-800 flex justify-between items-center">
                                                <span className="text-[8px] font-black uppercase tracking-widest text-slate-500 italic">Análise gerada via Modelo Gemini 3 Flash</span>
                                                <span className="px-3 py-1 bg-emerald-500/10 text-emerald-500 rounded-full text-[8px] font-black uppercase tracking-widest">Parecer Estratégico</span>
                                            </div>
                                        </div>
                                    ) : (
                                        <div className="bg-slate-50 p-6 rounded-[2rem] border border-dashed text-center">
                                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Clique em "Gerar Parecer Técnico" para análise estratégica.</p>
                                        </div>
                                    )}
                                </div>
                            </div>
                        ...
                        <div className="flex gap-2 w-full sm:w-auto">
                            {step < 5 ? (
                                ...
                                <button onClick={handleSave} className="w-full sm:w-auto px-12 py-4 bg-emerald-600 text-white rounded-2xl font-black uppercase text-[10px] shadow-lg flex items-center justify-center gap-2 hover:bg-emerald-700 transition-all">
                                    <Save size={18} /> {editingId ? 'Atualizar' : 'Salvar Investigação'}
                                </button>
                            )}
                        </div>
                    </div>
                            ...
                            <div key={inv.id} className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm hover:shadow-lg transition-all animate-fade border-l-4 border-l-blue-500">
                                <div className="flex justify-between items-start mb-4">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <h4 className="font-black text-slate-800 text-sm truncate max-w-[120px]">{inv.occurrence.description || 'Sem descrição'}</h4>
                                            <div className="flex items-center gap-1 text-[9px] text-slate-400 font-bold uppercase tracking-widest"><Calendar size={10} /> {inv.occurrence.date}</div>
                                        </div>
                                    </div>
                                    <div className="flex gap-1">
                                        <button onClick={() => handleEdit(inv)} className="text-slate-300 hover:text-blue-500 transition-colors p-1" title="Editar"><Pencil size={16} /></button>
                                        <button onClick={() => deleteInvestigation(inv.id)} className="text-slate-200 hover:text-red-500 transition-colors p-1" title="Excluir"><Trash2 size={16} /></button>
                                    </div>
                                </div>
                                ...
                                <div className="mt-4 flex justify-between items-center">
                                    <span className={`px-3 py-1 rounded-full text-[8px] font-black uppercase tracking-tighter ${inv.twttpAdvanced ? 'bg-amber-50 text-amber-600' : 'bg-emerald-50 text-emerald-600'}`}>{inv.twttpAdvanced ? 'Via TWTTP Avançada' : 'Via HERCA'}</span>
