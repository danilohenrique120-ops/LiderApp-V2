...
const HumanErrorView: React.FC<HumanErrorViewProps> = ({ investigations, user, db }) => {
    ...
    // Lógica de Roteamento (Respeitando a regra TWTTP Avançada -> Plano de Ação)
    const handleNextStep = () => {
        ...
        else if (step === 3) setStep(5); // Avançada vai direto para Plano
    ...
    const handleGenerateAiReport = async () => {
        ...
        try {
            ...
            const prompt = `
                Atue como Especialista Sênior em Segurança Industrial.
                Analise esta investigação de erro humano:
                ENVOLVIDO: ${occurrence.employee.name} (${occurrence.employee.function})
                OCORRÊNCIA: ${occurrence.description}
                METODO: ${needsAdvanced ? 'TWTTP Avançada (Gap de Conhecimento)' : 'HERCA (Fatores Contribuintes)'}
                PLANO: ${actionPlan.action} (Causa: ${actionPlan.rootCauses})
                CONTRAMEDIDA: ${actionPlan.countermeasure === 'Outros' ? customCountermeasure : actionPlan.countermeasure}
                
                GERAR:
                1. RESUMO EXECUTIVO DO RISCO.
                2. VALIDAÇÃO TÉCNICA DA CONTRAMEDIDA PROPOSTA.
                3. RECOMENDAÇÃO ESTRATÉGICA PARA O LÍDER.
            `;
            const response = await ai.models.generateContent({
                model: 'gemini-3-flash-preview',
                contents: prompt,
            });
            if (response.text) setAiReport(response.text);
        } catch (error: any) {
    ...
    const handleEdit = (inv: HumanErrorInvestigation) => {
        ...
        // Verifica se a contramedida salva é customizada
                    ...
                    <div className="max-h-[65vh] overflow-y-auto pr-2 custom-scrollbar">
                            ...
                            <div className="space-y-6 animate-fade">
                                ...
                                <div className="mt-8 pt-6 border-t">
                                    <div className="flex items-center justify-between mb-4">
                                        <h4 className="text-[10px] font-black text-blue-600 uppercase tracking-widest flex items-center gap-2"><Sparkles size={16}/> Inteligência Artificial (Análise Gemini)</h4>
                                        <button onClick={handleGenerateAiReport} disabled={isAiLoading} className="text-[9px] font-black text-blue-600 underline">Gerar Análise Crítica</button>
                                    </div>
                                    {isAiLoading && <div className="p-8 bg-slate-50 text-center animate-pulse text-[10px] uppercase font-black text-slate-400">Consultando Base de Dados IA...</div>}
                                    {aiReport && (
                                        <div className="bg-slate-900 text-slate-300 p-6 rounded-2xl text-xs whitespace-pre-wrap leading-relaxed relative animate-fade shadow-2xl">
                                            <button onClick={handleCopyReport} className="absolute top-4 right-4 text-white/50">{copied ? <Check size={14} className="text-emerald-400"/> : <Copy size={14}/>}</button>
                                            {aiReport}
                                        </div>
                                    )}
                                </div>
                        ...
                        {step === 6 && (
                            <div className="space-y-8 animate-fade pb-10">
                                ...
                                <div className="bg-slate-100 p-8 rounded-[2.5rem] border overflow-x-auto shadow-inner">
                                    <div id="pdf-report-container" className="bg-white p-12 shadow-2xl mx-auto min-w-[210mm] max-w-[210mm] min-h-[297mm] text-slate-800 font-sans border border-slate-200">
