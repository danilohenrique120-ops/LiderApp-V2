...
const HumanErrorView: React.FC<HumanErrorViewProps> = ({ investigations, user, db }) => {
    ...
    const handleGeneratePDF = async () => {
        ...
        const opt = {
            margin: [10, 10, 10, 10], 
            filename: `INVEST_DESVIO_${occurrence.employee.name.replace(/\s+/g, '_')}_${occurrence.date}.pdf`,
    ...
    return (
        <div className="animate-fade">
            <header className="mb-10 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h2 className="text-3xl font-black text-slate-100 tracking-tight uppercase">DESVIOS OPERACIONAIS</h2>
                    <p className="text-slate-400 font-medium">Investigação técnica Método Sistema Líder.</p>
                </div>
                {!showForm && (
                    <button 
                                ...
                                <div className="bg-slate-100 p-8 rounded-[2.5rem] border shadow-inner">
                                    ...
                                    >
                                        ...
                                        <div style={{ pageBreakInside: 'avoid', marginBottom: '20px' }} className="flex justify-between items-center border-b-4 border-slate-900 pb-6">
                                            <div>
                                                <h1 className="text-3xl font-black uppercase tracking-tighter">RELATÓRIO DE DESVIOS OPERACIONAIS</h1>
