...
const HumanErrorView: React.FC<HumanErrorViewProps> = ({ investigations, user, db }) => {
    ...
    const handleNextStep = () => {
        ...
        if (step === 1) setStep(2);
            ...
            if (advancedYes) setStep(5); 
            else setStep(4); 
        }
        else if (step === 4) setStep(5); 
        else if (step === 5) setStep(6); // Novo passo de Revisão
    };
    ...
    const handleGeneratePDF = async () => {
        ...
        const opt = {
            margin: [10, 10],
            filename: `INVESTIGACAO_EH_${occurrence.employee.name.replace(/\s+/g, '_')}_${occurrence.date}.pdf`,
        ...
        try {
            ...
            alert("Falha ao processar o PDF. Tente novamente.");
        } finally {
            setIsGeneratingPdf(false);
        }
    };
    ...
    const handleGenerateAiReport = async () => {
        if (!occurrence.description) {
            alert("Por favor, preencha a descrição do problema antes de gerar a análise.");
            return;
        }
        setIsAiLoading(true);
        setAiReport(null);
        try {
            const response = await fetch('/api/analyze', {
                ...
                body: JSON.stringify({ occurrence, twttp, herca, actionPlan })
            });
            if (!response.ok) throw new Error('Erro na comunicação com a IA.');
            const data = await response.json();
            if (data.text) setAiReport(data.text);
        } catch (error: any) {
        ...
        <div className="animate-fade">
            <header className="mb-10 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    ...
                    <p className="text-slate-500 font-medium">Investigação técnica e análise de contramedidas.</p>
                </div>
                {!showForm && (
                    <button 
                ...
                <div className="bg-white p-8 rounded-[2.5rem] border shadow-2xl animate-fade mb-10 overflow-hidden relative">
                    ...
                    <div className="flex justify-between items-center mb-10 border-b pb-6 overflow-x-auto custom-scrollbar gap-4">
                        <div className="flex items-center gap-2">
                            {[1, 2, needsAdvanced ? 3 : null, 4, 5, 6].filter(Boolean).map((s, idx) => (
                    ...
                    <div className="max-h-[65vh] overflow-y-auto pr-2 custom-scrollbar">
                        {step === 1 && (
                            <div className="space-y-6 animate-fade">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div><label className="label-Invest">Local Exato</label><input className="input-Invest" value={occurrence.failureLocation} onChange={e => setOccurrence({...occurrence, failureLocation: e.target.value})} /></div>
                                </div>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 border-t pt-6">
                                    <div className="space-y-4">
                                        <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Colaborador Envolvido</h4>
                                    ...
                                    <div className="space-y-4">
                                        <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Entrevistador</h4>
                        ...
                        {step === 4 && (
                            <div className="space-y-8 animate-fade pb-10">
                                <h3 className="text-sm font-black text-blue-600 uppercase tracking-widest flex items-center gap-2"><ShieldAlert size={18}/> Fatores HERCA</h3>
                                {Object.entries({
                                    '1. PROCESSO': herca.process,
                                    '2. PROCEDIMENTO': herca.procedure,
                                    '3. FERRAMENTA': herca.tools,
                                    '4. AMBIENTE': herca.workplace,
                                    '5. ATITUDE': herca.attitude,
                                    '6. DESATENÇÃO': herca.inattention
                                }).map(([title, categoryData], idx) => (
                                    <div key={title} className="bg-slate-50 p-6 rounded-[2rem] border border-slate-100">
                                        <h4 className="text-[10px] font-black text-blue-600 uppercase mb-4">{title}</h4>
                                        <div className="space-y-3">
                                            {Object.keys(categoryData).map(qId => (
                                                <div key={qId} className="flex justify-between items-center py-2 border-b border-slate-200/50 last:border-0">
                                                    <p className="text-xs font-bold text-slate-600">Questão {qId}</p>
                                                    <div className="flex gap-2">
                                                        {['sim', 'não'].map(opt => (
                                                            <button key={opt} onClick={() => {
                                                                const category = (['process', 'procedure', 'tools', 'workplace', 'attitude', 'inattention'])[idx];
                                                                setHerca({...herca, [category]: {...(herca as any)[category], [qId]: opt}});
                                                            }} className={`px-4 py-1.5 rounded-lg text-[9px] font-black uppercase ${(herca as any)[(['process', 'procedure', 'tools', 'workplace', 'attitude', 'inattention'])[idx]][qId] === opt ? 'bg-blue-600 text-white' : 'bg-white text-slate-400 border'}`}>{opt}</button>
                                                        ))}
                                                    </div>
                                                </div>
                                            ))}
                        ...
                        {step === 5 && (
                            <div className="space-y-6 animate-fade">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div className="md:col-span-2"><label className="label-Invest">Causas Raízes</label><textarea className="input-Invest" rows={2} value={actionPlan.rootCauses} onChange={e => setActionPlan({...actionPlan, rootCauses: e.target.value})} /></div>
                                    <div className="md:col-span-2"><label className="label-Invest">Ação Corretiva</label><textarea className="input-Invest" rows={3} value={actionPlan.action} onChange={e => setActionPlan({...actionPlan, action: e.target.value})} /></div>
                                ...
                                <div className="mt-4 pt-6 border-t">
                                    <div className="flex items-center justify-between mb-4">
                                        <h4 className="text-[10px] font-black text-blue-600 uppercase tracking-widest flex items-center gap-2"><Sparkles size={16}/> Análise com IA Gemini</h4>
                                        <button onClick={handleGenerateAiReport} className="text-[9px] font-black text-blue-600 underline">Gerar Insight</button>
                                    </div>
                                    {isAiLoading && <div className="p-8 bg-slate-50 text-center animate-pulse text-[10px] uppercase font-black text-slate-400">Processando via Servidor...</div>}
                                    {aiReport && <div className="bg-slate-900 text-slate-300 p-6 rounded-2xl text-xs whitespace-pre-wrap leading-relaxed relative"><button onClick={handleCopyReport} className="absolute top-4 right-4 text-white/50">{copied ? <Check size={14}/> : <Copy size={14}/>}</button>{aiReport}</div>}
                                </div>
                            </div>
                        )}
                        ...
                        {step === 6 && (
                            <div className="space-y-8 animate-fade pb-10">
                                <div className="flex justify-between items-center">
                                    <h3 className="text-sm font-black text-slate-800 uppercase tracking-widest flex items-center gap-2"><Eye size={18}/> Resumo Técnico do Documento</h3>
                                    <button onClick={handleGeneratePDF} disabled={isGeneratingPdf} className="bg-emerald-600 text-white px-6 py-3 rounded-xl font-black uppercase text-[10px] flex items-center gap-2 shadow-lg disabled:opacity-50">
                                ...
                                <div className="bg-slate-50 p-8 rounded-[2rem] border border-slate-200 shadow-inner">
                                    <div id="pdf-report-container" className="bg-white p-10 shadow-2xl mx-auto max-w-[210mm] min-h-[297mm] text-slate-800 font-sans border border-slate-100">
                                        {/* Cabeçalho do PDF */}
                                        <div className="flex justify-between items-center border-b-2 border-slate-900 pb-4 mb-8">
                                            <div>
                                                <h1 className="text-2xl font-black uppercase tracking-tighter">RELATÓRIO DE INVESTIGAÇÃO</h1>
                                                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Desvio de Erro Humano - Método Sistema Líder</p>
                                            </div>
                                            <div className="text-right">
                                                <p className="text-[10px] font-black uppercase">Status: <span className="text-blue-600">Completo</span></p>
                                                <p className="text-[10px] font-black uppercase text-slate-400">Emissão: {new Date().toLocaleDateString()}</p>
                                            </div>
                                        </div>
                                        ...
                                        <div className="grid grid-cols-2 gap-8 mb-8 border-b pb-8">
                                            <div className="space-y-4">
                                                <h2 className="text-[10px] font-black uppercase bg-slate-900 text-white px-3 py-1 inline-block">1. Ocorrência</h2>
                                                <p className="text-xs"><strong>Descrição:</strong> {occurrence.description}</p>
                                                <p className="text-xs"><strong>Data/Hora:</strong> {occurrence.date} {occurrence.time}</p>
                                                <p className="text-xs"><strong>Local:</strong> {occurrence.unit} / {occurrence.area} ({occurrence.failureLocation})</p>
                                            </div>
                                            <div className="space-y-4">
                                                <h2 className="text-[10px] font-black uppercase bg-slate-900 text-white px-3 py-1 inline-block">2. Envolvidos</h2>
                                                <p className="text-xs"><strong>Colaborador:</strong> {occurrence.employee.name} ({occurrence.employee.function})</p>
                                                <p className="text-xs"><strong>Entrevistador:</strong> {occurrence.interviewee.name} ({occurrence.interviewee.function || occurrence.interviewee.area})</p>
                                            </div>
                                        </div>

                                        <div className="mb-8">
                                            <h2 className="text-[10px] font-black uppercase bg-slate-900 text-white px-3 py-1 inline-block mb-4">3. Análise de Conhecimento (TWTTP)</h2>
                                            <table className="w-full border-collapse border border-slate-200">
                                                <thead>
                                                    <tr className="bg-slate-50">
                                                        <th className="border border-slate-200 p-2 text-[10px] text-left">Pergunta</th>
                                                        <th className="border border-slate-200 p-2 text-[10px] text-center w-32">Status</th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    {Object.entries(twttp).map(([id, status]) => (
                                                        <tr key={id}>
                                                            <td className="border border-slate-200 p-2 text-[9px] font-medium leading-tight">Pergunta TWTTP #{id}</td>
                                                            <td className={`border border-slate-200 p-2 text-[9px] text-center font-black uppercase ${status === 'falta conhecimento' ? 'text-red-600' : 'text-emerald-600'}`}>{status}</td>
                                                        </tr>
                                                    ))}
                                                </tbody>
                                            </table>
                                        ...
                                        {needsAdvanced && (
                                            <div className="mb-8">
                                                <h2 className="text-[10px] font-black uppercase bg-amber-500 text-white px-3 py-1 inline-block mb-4">4. TWTTP Avançada</h2>
                                                <div className="grid grid-cols-1 gap-2">
                                                    {Object.entries(twttpAdvanced).filter(([_, v]) => v === 'sim').map(([id, _]) => (
                                                        <div key={id} className="text-[9px] bg-amber-50 p-2 border-l-4 border-amber-400">
                                                            <strong>Falha detectada no critério #{id}:</strong> Treinamento ou habilidade insuficiente identificada.
                                                        </div>
                                                    ))}
                                                </div>
                                            </div>
                                        ...
                                        <div className="mb-8">
                                            <h2 className="text-[10px] font-black uppercase bg-blue-600 text-white px-3 py-1 inline-block mb-4">5. Fatores de Risco HERCA</h2>
                                            <div className="flex flex-wrap gap-2">
                                                {Object.entries(herca).map(([cat, q]) => (
                                                    Object.entries(q).filter(([_, v]) => v === 'sim').map(([qId, _]) => (
                                                        <span key={`${cat}-${qId}`} className="px-2 py-1 bg-blue-50 text-blue-700 text-[8px] font-black border border-blue-100 rounded">
                                                            Risco #{qId} em {cat.toUpperCase()}
                                                        </span>
                                                    ))
                                                )).flat()}
                                                {Object.values(herca).every(cat => Object.values(cat).every(v => v === 'não')) && (
                                                    <p className="text-[9px] italic text-slate-400">Nenhum fator crítico HERCA detectado para esta investigação.</p>
                                                )}
                                            </div>
                                        </div>

                                        <div className="bg-slate-50 p-6 border-t-2 border-slate-900">
                                            <h2 className="text-[10px] font-black uppercase bg-emerald-600 text-white px-3 py-1 inline-block mb-4">6. Plano de Ação & Contramedida</h2>
                                            <div className="grid grid-cols-2 gap-6">
                                                <div>
                                                    <p className="text-[10px] font-black uppercase text-slate-400">Causa Raiz:</p>
                                                    <p className="text-xs font-bold mb-4">{actionPlan.rootCauses}</p>
                                                    <p className="text-[10px] font-black uppercase text-slate-400">Contramedida:</p>
                                                    <p className="text-xs font-black text-blue-600">{actionPlan.countermeasure}</p>
                                                </div>
                                                <div>
                                                    <p className="text-[10px] font-black uppercase text-slate-400">Ação Corretiva:</p>
                                                    <p className="text-xs mb-4">{actionPlan.action}</p>
                                                    <div className="flex justify-between">
                                                        <div><p className="text-[10px] font-black uppercase text-slate-400">Responsável:</p><p className="text-xs font-bold">{actionPlan.responsible}</p></div>
                                                        <div><p className="text-[10px] font-black uppercase text-slate-400">Prazo:</p><p className="text-xs font-bold">{actionPlan.deadline}</p></div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                            ...
                            <div key={inv.id} className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm hover:shadow-lg transition-all animate-fade border-l-4 border-l-blue-500">
                                <div className="flex justify-between items-start mb-4">
                                    ...
                                    <div className="flex gap-1">
                                        ...
                                        {/* Fixed: Replaced undefined deleteInvestigation with direct Firestore delete call and confirmation */}
                                        <button onClick={() => { if(confirm('Excluir esta investigação?')) db.collection('human_error_investigations').doc(inv.id).delete() }} className="text-slate-300 hover:text-red-500 p-1"><Trash2 size={16}/></button>
                                    </div>
                                </div>
                                <div className="space-y-2 py-4 border-y border-slate-50">
                                    <div className="flex justify-between text-[10px] font-bold"><span className="text-slate-400 uppercase">Colaborador:</span><span className="text-slate-700">{inv.occurrence.employee.name}</span></div>
                                    <div className="flex justify-between text-[10px] font-bold"><span className="text-slate-400 uppercase">Plano:</span><span className="text-blue-600">{inv.actionPlan.countermeasure}</span></div>
                                </div>
                                <div className="mt-4 flex justify-between items-center">
                                    <span className="px-2 py-1 bg-slate-100 text-[8px] font-black uppercase rounded text-slate-500">{inv.occurrence.unit}</span>
                                    <button onClick={() => handleEdit(inv)} className="text-[9px] font-black text-blue-600 uppercase flex items-center gap-1">Visualizar <Eye size={10}/></button>
                                </div>
                            </div>
                        ))}
                    </div>
            ...
            <style>{`
                ...
                .input-Invest { width: 100%; padding: 0.75rem 1rem; background-color: #f8fafc; border: 1px solid #e2e8f0; border-radius: 1rem; outline: none; font-size: 13px; }
