...
const DdsView: React.FC<DdsViewProps> = ({ investigations, user, db }) => {
    ...
    const handleGenerateDds = async () => {
        ...
        try {
            // Cobrança
            const hasCredits = await aiService.consumeCredits(user, db, 1, 'DDS');
            if (!hasCredits) {
                alert("Você não possui Líder Coins suficientes (Custo: 1). Por favor, realize um upgrade do seu plano.");
