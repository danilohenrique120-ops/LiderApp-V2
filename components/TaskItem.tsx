...
const TaskItem: React.FC<TaskItemProps> = ({
  ...
  const deadlineDate = hasDeadline ? parseISO(task.deadline as string) : new Date();

  const isOverdue = hasDeadline && task.status !== 'done' && !task.completed && isBefore(deadlineDate, today);
  const isNear = hasDeadline && task.status !== 'done' && !task.completed && (isToday(deadlineDate) || isTomorrow(deadlineDate));
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-4 ml-4 shrink-0">
        {/* Priority Badge */}
        {task.priority !== 'Nenhuma' && task.priority && (
        ...
        <div className={`flex items-center gap-2 text-xs font-bold whitespace-nowrap min-w-[95px] px-3 py-1.5 rounded-full transition-all ${!task.deadline ? 'text-slate-400 bg-slate-50' : 
          isDone ? 'text-slate-400 bg-slate-50' :
