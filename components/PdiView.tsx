...
const PdiView: React.FC<PdiViewProps> = ({ pdis, employees, user, db }) => {
    ...
    const handleSave = async (e: React.FormEvent) => {
        e.preventDefault();
        if (formData.goals.length === 0 && (!formData.mainGoals || formData.mainGoals.length === 0)) { alert('Adicione ao menos uma meta.'); return; }

        if (!editingId) {
            // Cobrança de Créditos na Criação de novo PDI (8)
