...
export class AiService {
  ...
  /**
   * SISTEMA DE COBRANÇA (Líder Coins)
   */
  public async consumeCredits(user: any, db: any, cost: number, feature: string): Promise<boolean> {
    if (!user || user.plan === 'pro') return true; // PRO é ilimitado em DDS, mas e análise de causa?
    // Regra da Página de Vendas: "DDS ILIMITADO: Roteiros sem custo de créditos [PRO]"
    // Porém: PDI e RCA custam créditos para todo mundo, afinal PRO só tem 200 mensal, Start tem 50.
    // Exceção do Pro: Se feature === 'DDS', custa 0.
    
    let actualCost = cost;
    if (user.plan === 'pro' && feature === 'DDS') {
        actualCost = 0;
    }
    ...
    await db.collection('users').doc(user.uid).update({
        credits: firebase.firestore.FieldValue.increment(-actualCost)
    });
  ...
  /**
   * IA Q&A DE DOCUMENTOS DE SEGURANÇA E COMPLIANCE
   */
  public async queryKnowledgeBase(question: string, context: string): Promise<string> {
    if (!this.isEnabled || !this.ai) return "Serviço de IA indisponível (Verifique sua API Key).";
    try {
      const response = await this.ai.models.generateContent({
        model: 'gemini-2.5-flash',
        contents: `CONTEXTO DOS DOCUMENTOS INTERNOS (POPs, Manuais, Treinamentos):\n${context}\n\nPERGUNTA DO GESTOR:\n${question}`,
        config: {
          systemInstruction: "Você é o Assistente EHS e Especialista de Compliance do sistema. Sua missão é responder perguntas ESTRITAMENTE com base no contexto (documentos anexados) fornecido. Caso a resposta não exista no material, diga claramente que não foi encontrada nos documentos anexados. Formate bem o texto com negritos e subtópicos rápidos para fácil leitura técnica."
        }
      });
      return response.text || "Não foi possível gerar uma resposta coerente.";
    } catch (error) {
      console.error("AI QA Error:", error);
      return "Houve um erro ao processar seus manuais. Tente reformular a pergunta.";
    }
  }
  ...
  public async generateDds(prompt: string, config: { duration: number, tone: string }): Promise<DdsResponse> {
    ...
    try {
      ...
      throw new Error(`Falha ao gerar DDS técnico. Detalhe: ${error.message}`);
    }
  }
