...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // History Filter based on Plan
    const filterByPlanHistory = (docs: any[], dateField: string = 'createdAt') => {
      const plan = userData?.plan || 'free';
      if (plan === 'pro') return docs; // Vitalício
      
      const limitDays = plan === 'start' ? 30 : 7; // Start: 30, Free: 7
      const cutoff = new Date(new Date().getTime() - limitDays * 24 * 60 * 60 * 1000);

      return docs.filter(d => {
        const item = d.data ? d.data() : d; // Handle both DocumentSnapshot and mapped objects
        if (!item[dateField]) return true; // Keep items without dates like settings
        const itemDate = item[dateField]?.seconds ? new Date(item[dateField].seconds * 1000) : new Date(item[dateField]);
        return itemDate >= cutoff;
      });
    };

    const unsubs = [
      db.collection('operators').where(...filter).onSnapshot(s => setMatrixData(filterByPlanHistory(s.docs).map(d => ({ ...d.data(), id: d.id }) as Operator))),
      db.collection('skills_config').where(...filter).onSnapshot(s => setSkills(s.docs.map(d => ({ ...d.data(), id: d.id }) as SkillConfig))), // No filter for config
      db.collection('pdis').where(...filter).onSnapshot(s => setPdis(filterByPlanHistory(s.docs).map(d => ({ ...d.data(), id: d.id }) as PDI))),
      db.collection('meetings').where(...filter).onSnapshot(s => setMeetings(filterByPlanHistory(s.docs).map(d => ({ ...d.data(), id: d.id }) as Meeting))),
      db.collection('procedures').where(...filter).onSnapshot(s => setProcedures(s.docs.map(d => ({ ...d.data(), id: d.id }) as Procedure))), // Configuration data
      db.collection('training_records').where(...filter).onSnapshot(s => {
        const recs: Record<string, TrainingRecord> = {};
        filterByPlanHistory(s.docs, 'date').forEach((d: any) => { recs[d.id] = d.data() as TrainingRecord });
        setTrainingRecords(recs);
      }),
      db.collection('production_log').where(...filter).onSnapshot(s => {
        const data = filterByPlanHistory(s.docs, 'date').map((d: any) => ({ ...d.data(), id: d.id }) as ProductionEntry);
      ...
      db.collection('human_error_investigations').where(...filter).onSnapshot(s => {
        const data = filterByPlanHistory(s.docs).map((d: any) => ({ ...d.data(), id: d.id }) as HumanErrorInvestigation);
      ...
      db.collection('todo_tasks').where(...filter).onSnapshot(s => setTodoTasks(filterByPlanHistory(s.docs).map((d: any) => ({ ...d.data(), id: d.id }) as TodoTask))),
      db.collection('todo_notes').where(...filter).onSnapshot(s => {
        const notes = filterByPlanHistory(s.docs).map((d: any) => ({ ...d.data(), id: d.id }) as TodoNote);
      ...
      db.collection('follow_up_items').where(...filter).onSnapshot(s => setFollowUpItems(filterByPlanHistory(s.docs).map((d: any) => ({ ...d.data(), id: d.id }) as FollowUpItem))),
      db.collection('knowledge_docs').where(...filter).onSnapshot(s => setKnowledgeDocs(s.docs.map(d => ({ ...d.data(), id: d.id }) as KnowledgeDoc))),
      db.collection('rca_investigations').where(...filter).onSnapshot(s => setRcaInvestigations(filterByPlanHistory(s.docs).map((d: any) => ({ ...d.data(), id: d.id }) as RCAInvestigation))),
      ...
      <main className="flex-1 overflow-y-auto min-h-screen custom-scrollbar">
        ...
        <div className="p-6 md:p-12 w-full pb-32">
          ...
          {activeTab === 'cause-analysis' && (
            <CauseAnalysisView
              investigations={rcaInvestigations}
              user={user}
              db={db}
            />
          )}
